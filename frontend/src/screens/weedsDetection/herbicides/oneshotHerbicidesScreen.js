...
export default function OneShotHerbicidesScreen() {
...
const [filteredData, setFilteredData] = useState([]);
  ...
  useEffect(() => {
    const unsub = db
      ...
      .where('category', '==', 'One-shot herbicides')
      .onSnapshot(snapshot => {
...
const handleSearch = (text) => {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>🎯 One-shot Herbicides</Text>
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 24, fontWeight: 'bold', marginBottom: 12, color: '#1e40af' },
  card: { 
    backgroundColor: '#e2fdd9ff', 
    padding: 16, 
    borderRadius: 12, 
    marginBottom: 12, 
    shadowColor: "#000",
