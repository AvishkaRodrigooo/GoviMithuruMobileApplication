...
# Fix the data.yaml path to use absolute path
...
if os.path.exists(resume_path):
    ...
    try:
        # Load the checkpoint to get current epoch
        ...
        print("   ⚠️ Could not determine current epoch from checkpoint")
    ...
    if remaining_epochs <= 0:
        ...
        # Train for exactly the remaining epochs
        results = model.train(
            data=data_yaml,
            epochs=remaining_epochs,  # Train only the remaining epochs
            imgsz=416,
            batch=32,
            workers=4,
            cache=True,
            ...
            verbose=True,
            project='runs/train',
            resume=False  # Important: set to False to use our epoch calculation
        )
else:
...
if os.path.exists('models/pest_3class.pt'):
    ...
    print(f"\n⚠️ Model not found for validation")
