...
try:
    ...
    print(" pymongo not installed - running without database")
    app.db = None
...
try:
    ...
    if not model_loaded:
        print(" No YOLO model found in any path. Please train the model first.")
        print("   Looked in:")
        ...
        if hasattr(app.model, 'names'):
            ...
            if len(model_classes) == 3:
                ...
                print(f" Model has {len(model_classes)} classes, but API expects 3 classes")
        ...
        try:
            ...
            print(f" Model test inference failed: {e}")
...
# Load Enhanced Pest Forecast Models 
...
try:
    ...
    if os.path.exists(complete_model_path):
        ...
        if len(existing_files) == len(required_files):
            ...
            print(" Pest forecast model files not found. Some files are missing:")
    ...
    print(f" Failed to load Pest Forecast Models: {e}")
    import traceback
    traceback.print_exc()
    app.pest_model = None
...
try:
    ...
    print(f" Failed to register postharvest_guardian: {e}")
...
try:
    ...
    print(f" Failed to register predict: {e}")
...
try:
    ...
    print(f" Failed to register weed_predict: {e}")
...
try:
    ...
    print(f" Failed to register pest_routes: {e}")
...
try:
    ...
    print(f" Failed to register pest_detection_routes: {e}")
...
try:
    ...
    print(f" Failed to register gemini_routes: {e}")
